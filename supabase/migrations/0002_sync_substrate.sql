-- KIUBO Cloud v1 · idempotent offline synchronization substrate
...
create or replace function public.pull_sync_changes(p_cursor timestamptz default '1970-01-01T00:00:00Z')
returns jsonb language plpgsql security definer set search_path=public as $$
...
begin
  ...
  into v_next,v_changes from public.sync_entities where updated_at>v_cursor and public.has_tenant_access(tenant_id) and(branch_id is null or public.has_branch_access(tenant_id,branch_id));
  return jsonb_build_object('cursor',coalesce(v_next,v_cursor),'changes',v_changes);
end$$;
revoke all on function public.pull_sync_changes(timestamptz) from public;grant execute on function public.pull_sync_changes(timestamptz) to authenticated;
