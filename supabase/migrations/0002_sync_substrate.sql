...
create table public.sync_entities(
  ...
  entity_type text not null,entity_id text not null,payload jsonb not null default '{}'::jsonb,
...
create table public.sync_receipts(
  tenant_id uuid not null references public.tenants(id) on delete cascade,
  operation_id text not null,entity_type text not null,entity_id text not null,processed_at timestamptz not null default now(),
...
returns boolean language sql stable security definer set search_path=public as $$
  select public.is_platform_admin() or case
    ...
    when target_type in('customers','sales','cashSessions','cashMovements','credits','creditPayments') then public.has_tenant_role(target_tenant,array['owner','admin','cashier'])
...
begin
  ...
  for op in select value from jsonb_array_elements(coalesce(p_operations,'[]'::jsonb)) loop
    begin
      ...
      if v_type not in('tenantProducts','customers','sales','cashSessions','cashMovements','credits','creditPayments','settings','branding','suppliers','purchases','supplierPayments','stockMovements') then raise exception 'unsupported entity type';end if;
      ...
      if exists(select 1 from public.sync_receipts where tenant_id=v_tenant and operation_id=v_operation) then
        v_results=v_results||jsonb_build_array(jsonb_build_object('operationId',v_operation,'ok',true,'duplicate',true));continue;
      end if;
...
begin
  ...
  select max(updated_at),coalesce(jsonb_agg(jsonb_build_object(
    'tenantId',tenant_id::text,'branchId',branch_id::text,'entityType',entity_type,'entityId',entity_id,
    'action',case when deleted then 'delete' else 'upsert' end,'payload',case when deleted then null else payload end,'updatedAt',updated_at
  ) order by updated_at),'[]'::jsonb)
  into v_next,v_changes from public.sync_entities
  where tenant_id=p_tenant and updated_at>v_cursor and(branch_id is null or public.has_branch_access(tenant_id,branch_id));
  return jsonb_build_object('cursor',coalesce(v_next,v_cursor),'changes',v_changes);
end$$;
revoke all on function public.pull_sync_changes(uuid,timestamptz) from public;
grant execute on function public.pull_sync_changes(uuid,timestamptz) to authenticated;
