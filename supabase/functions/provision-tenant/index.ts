...
Deno.serve(async(req)=>{
  ...
  const invited=await admin.auth.admin.inviteUserByEmail(ownerEmail,{data:{full_name:ownerName,business_name:businessName,kiubo_role:"owner"},redirectTo});
  ...
  if(provision.error){await admin.auth.admin.deleteUser(userId).catch(()=>undefined);return reply({error:provision.error.message},400)}
  const row=Array.isArray(provision.data)?provision.data[0]:provision.data;
  if(!row?.tenant_id||!row?.branch_id){await admin.auth.admin.deleteUser(userId).catch(()=>undefined);return reply({error:"tenant provisioning did not return identifiers"},500)}
