...
async function upload(row,file){const ui=$('.v77-presentation-image',row);ui?.classList.add('busy');try{const {pid,p}=await ensureIdentity(row);if(!pid||!p?.id)throw new Error('presentation_not_saved');row.dataset.id=String(p.id);const info=rowInfo(row),current=metaForRow(pid,row)||{},f=await optimize(file),fd=new FormData();fd.set('action','upload_asset');fd.set('kind','product');fd.set('slug',slug);fd.set('old_path',current.p||p.image_path||'');fd.set('file',f,f.name);const r=await upstreamFetch(C.apiUrl,{method:'POST',headers:{'X-Client-Version':C.version,'X-Provider-Session':token()},body:fd});const up=await r.json().catch(()=>({}));if(!r.ok)throw new Error(up.error||'upload_failed');const map={...mapFor(pid)};map[String(p.id)]={u:up.public_url||'',p:up.path||'',n:info.name,c:info.units};imageMaps.set(String(pid),map);row.dataset.v77PendingImage='1';updateControl(row);toast(`Foto de ${info.name||'presentación'} lista · guarda el producto`)}catch(err){console.error(err);if(err?.message==='presentation_not_saved')toast('Guarda el producto una vez para crear esa presentación y luego sube la foto','error');else toast(err?.message==='file_too_large'?'La imagen supera 5 MB':'No pudimos subir esa imagen','error')}finally{ui?.classList.remove('busy')}}
