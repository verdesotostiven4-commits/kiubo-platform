/* Hakuna Catalog 10.16.6 — smooth brand marquee + catalog position preservation. */
(()=>{
'use strict';
if(window.__hm1166Catalog)return;window.__hm1166Catalog=true;
...
let detailOrigin=null,detailWasOpen=false;
...
function matchingProducts(q){const n=norm(q);return visibleProducts().filter(p=>productMatches(p,q)).sort((a,b)=>{const an=norm(a.name),bn=norm(b.name),ab=norm(a.brand),bb=norm(b.brand);return Number(!(an.startsWith(n)||ab.startsWith(n)))-Number(!(bn.startsWith(n)||bb.startsWith(n)))||an.localeCompare(bn,'es')}).slice(0,40)}
...
function freshBrandRail(){let rail=$('.hm-v104-catalog-brands');if(!rail||rail.dataset.hm1166Fresh==='1')return rail;const copy=rail.cloneNode(true);copy.dataset.hm1166Fresh='1';copy.dataset.hm1072Bound='1';rail.replaceWith(copy);return copy}
function setupBrandMarquee(){const rail=freshBrandRail();if(!rail)return;removeLegacyArrows();const signature=[...rail.querySelectorAll(':scope > [data-brand-chip]')].map(b=>b.dataset.brandChip||'').join('|');if(rail.dataset.hm1166Signature===signature&&rail.querySelector('.hm1165-brand-clone'))return;rail.dataset.hm1166Signature=signature;$$(':scope > .hm1165-brand-clone',rail).forEach(x=>x.remove());const originals=[...rail.querySelectorAll(':scope > [data-brand-chip]')].filter(b=>String(b.dataset.brandChip||'').trim());if(originals.length<2)return;const frag=document.createDocumentFragment();for(const b of originals){const clone=b.cloneNode(true);const name=b.dataset.brandChip||'';clone.removeAttribute('data-brand-chip');clone.dataset.hm1165BrandClone=name;clone.classList.add('hm1165-brand-clone');clone.classList.remove('active');frag.append(clone)}rail.append(frag);const first=originals[0],cloneFirst=rail.querySelector(':scope > .hm1165-brand-clone');const seq=++marqueeSeq;let dragging=false,last=performance.now(),position=rail.scrollLeft;const speed=.06;const loopWidth=()=>cloneFirst&&first?cloneFirst.offsetLeft-first.offsetLeft:0;rail.addEventListener('pointerdown',()=>{dragging=true;position=rail.scrollLeft},{passive:true});const end=()=>{position=rail.scrollLeft;dragging=false;last=performance.now()};rail.addEventListener('pointerup',end,{passive:true});rail.addEventListener('pointercancel',end,{passive:true});rail.addEventListener('touchend',end,{passive:true});const tick=now=>{if(seq!==marqueeSeq||!rail.isConnected)return;const dt=Math.min(34,Math.max(0,now-last));last=now;const w=loopWidth();if(dragging){position=rail.scrollLeft}else if(document.visibilityState==='visible'&&w>0){position+=dt*speed;if(position>=w)position-=w;rail.scrollLeft=position}requestAnimationFrame(tick)};requestAnimationFrame(tick)}

function rememberDetailOrigin(target){const trigger=target?.closest?.('#catalogView.active [data-open]');if(!trigger)return;const card=trigger.closest?.('[data-product]');detailOrigin={y:window.scrollY,id:card?.dataset.product||'',top:card?.getBoundingClientRect?.().top??null}}
function restoreDetailOrigin(origin){if(!origin||!$('.v10-nav [data-view="catalog"].active'))return;const run=()=>{if($('#sheetHost .v102-detail'))return;let y=Number(origin.y||0);if(origin.id&&origin.top!=null){const card=$(`#catalogResults [data-product="${CSS.escape(String(origin.id))}"]`);if(card){const delta=card.getBoundingClientRect().top-Number(origin.top);y=window.scrollY+delta}}const prev=root.style.scrollBehavior;root.style.setProperty('scroll-behavior','auto','important');window.scrollTo(0,Math.max(0,y));requestAnimationFrame(()=>{if(prev)root.style.scrollBehavior=prev;else root.style.removeProperty('scroll-behavior')})};requestAnimationFrame(run);setTimeout(run,45);setTimeout(run,170)}
function syncDetailPosition(){const open=!!$('#sheetHost .v102-detail');if(open&&!detailWasOpen){detailWasOpen=true;if(!detailOrigin)detailOrigin={y:window.scrollY,id:'',top:null}}else if(!open&&detailWasOpen){detailWasOpen=false;const origin=detailOrigin;detailOrigin=null;restoreDetailOrigin(origin)}}
...
window.addEventListener('pageshow',()=>{scheduleUi();syncSearchFromInput();syncDetailPosition()});
window.addEventListener('storage',e=>{if(e.key===cacheKey())scheduleUi()});

function syncUi(){uiRaf=0;removeLegacyArrows();setupBrandMarquee();syncDetailPosition();const old=$('#searchSuggestions');if(old)old.setAttribute('aria-hidden','true');document.documentElement.dataset.hmCatalogClient='10.16.6'}
...
const start=()=>{ensureSearchPanel();scheduleUi();syncSearchFromInput();syncDetailPosition()};
