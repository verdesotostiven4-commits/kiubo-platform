/* Hakuna Panel 10.10 — provider helpers without self-triggering observers. */
(()=>{
'use strict';
...
function polishExistingReset(){const card=$('.v72-reset-card');if(!card)return;setText($('header p',card),'Limpia pedidos de prueba y empieza el historial desde cero. Productos, fotos, categorías y configuración se conservan; si un pedido ya había descontado stock, el inventario se repone automáticamente.');setText($('.v72-reset-body small',card),'Mantén presionado 5 segundos. Después todavía tendrás que confirmar.')}
function polishExistingDelete(){const zone=$('#v7DeleteProductZone');if(!zone)return;setText($('small',zone),'Para duplicados o productos que ya no manejarás. Se oculta del catálogo, pero los pedidos anteriores conservan su historial.')}
...
function enhance(){clearLegacyModalLock();ensureCategoryShortcut();polishExistingReset();polishExistingDelete();ensureOrderStockNote();document.documentElement.dataset.hmPanelHelper='10.10'}
