...
function syncInventoryUi(){
  ...
  const small=$('small',track);if(small){const text=!s.initialized?'Pendiente de conteo · al registrar la primera cantidad quedará protegido por stock.':s.tracking?'Activo · no permitirá confirmar pedidos por encima de lo disponible.':'Desactivado · este producto no limita pedidos por inventario.';if(small.textContent!==text)small.textContent=text}
...
function enhanceOrderModal(){
  ...
  if(order.status==='new'){b.textContent='Pendiente de confirmación';p.textContent='Todavía no descuenta inventario. Confirmar vuelve a revisar el stock y recién ahí lo descuenta.';actions.innerHTML='<button type="button" class="hm115-confirm">Confirmar pedido</button><button type="button" class="hm115-reject">Rechazar</button>';$('.hm115-confirm',actions).onclick=e=>changeOrder(order,'confirmed',e.currentTarget);$('.hm115-reject',actions).onclick=e=>changeOrder(order,'cancelled',e.currentTarget)}
  else if(['confirmed','preparing','dispatched','delivered'].includes(order.status)){b.textContent=order.status==='confirmed'?'Pedido confirmado':'Pedido en proceso';p.textContent='El inventario de este pedido ya fue comprometido. Si se cancela desde el estado del pedido, el stock se devuelve automáticamente.';actions.innerHTML='<span class="hm115-committed">Inventario descontado</span>'}
