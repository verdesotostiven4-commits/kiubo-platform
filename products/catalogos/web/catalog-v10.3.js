...
function preferredAvailable(existing){const map=new Map(existing.map(btn=>[norm(btn.dataset.brandChip||btn.textContent),btn]));const ordered=[];for(const name of preferredBrands){const btn=map.get(norm(name));if(btn)ordered.push(btn)}for(const btn of existing){if(!ordered.includes(btn)&&btn.dataset.brandChip)ordered.push(btn)}return ordered}
...
function decorateBrandRail(home){const block=$('.v10-brand-block',home);if(!block)return;const hero=$('.v10-hero',home);if(hero&&block.nextElementSibling!==hero)home.insertBefore(block,hero);
  ...
  const rail=$('.v10-brand-rail',block);if(!rail)return;let buttons=$$('[data-brand-chip]',rail);const all=buttons.find(b=>!b.dataset.brandChip);if(all)all.remove();buttons=buttons.filter(b=>b.dataset.brandChip);const ordered=preferredAvailable(buttons);rail.innerHTML='';for(const btn of ordered.slice(0,5)){const name=btn.dataset.brandChip;btn.className='hm-brand-chip';btn.innerHTML=brandLogo(name);btn.setAttribute('aria-label',`Ver productos ${name}`);rail.append(btn)}const more=document.createElement('button');more.type='button';more.className='hm-brand-more';more.dataset.hmOpenBrands='1';more.innerHTML='<span>•••</span><b>Más</b>';rail.append(more)}
...
function decorateBrandSheet(){const sheet=$('.v10-filter-sheet');if(!sheet||sheet.dataset.hmSheet)return;sheet.dataset.hmSheet='1';sheet.classList.add('hm-brand-sheet');const title=$('h2',sheet),desc=$('p',sheet),input=$('#brandSearch',sheet);if(title)title.textContent='Encuentra tu marca';if(desc)desc.textContent='Explora todas las marcas disponibles en el catálogo.';if(input)input.placeholder='Buscar marcas…';$$('[data-brand-option]',sheet).forEach((btn,i)=>{const name=btn.dataset.brandOption||'';btn.classList.add('hm-brand-option');btn.innerHTML=name?`${brandLogo(name)}<span><b>${esc(name)}</b><small>Ver productos</small></span><i>›</i>`:`<span class="hm-all-brands">${icons.grid}</span><span><b>Todas las marcas</b><small>Ver todo el catálogo</small></span><i>›</i>`})}
