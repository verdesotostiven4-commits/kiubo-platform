(() => {
  ...
  const sectionMarkup = (name, count) => {
  ...
  const organize = () => {
    ...
    for (const card of cards) {
      const name = card.querySelector(".product-category")?.textContent?.trim() || "Otros";
      if (!groups.has(name)) groups.set(name, []);
      groups.get(name).push(card);
    }

    const preferred = visibleCategoryOrder();
    const orderedNames = [
    ...
    for (const name of orderedNames) {
      ...
      section.append(sectionMarkup(name, products.length));

      const sectionGrid = document.createElement("div");
      sectionGrid.className = `catalog-section-grid${isList ? " list-grid" : ""}`;
      for (const card of products) sectionGrid.append(card);
  ...
  style.dataset.kiuboCatalogSections = "4.7";
  style.textContent = `
    ...
    .catalog-section-count{flex:0 0 auto;padding:6px 9px;border:1px solid var(--line);border-radius:999px;background:rgba(255,255,255,.68);color:var(--muted);font-size:10px;font-weight:750}
  ...
  document.addEventListener("click", event => {
    if (event.target.closest?.("[data-open-search]")) setTimeout(improveSearchCopy, 0);
  }, true);
