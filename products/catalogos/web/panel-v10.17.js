...
function pushState(){const ios=/iPhone|iPad|iPod/i.test(navigator.userAgent),standalone=window.matchMedia?.('(display-mode: standalone)').matches===true||navigator.standalone===true,supported='Notification'in window&&'serviceWorker'in navigator&&'PushManager'in window;if(ios&&!standalone&&!supported)return{kind:'install',label:'Instala el panel para recibir avisos',copy:'En iPhone: Safari → Compartir → Añadir a pantalla de inicio.'};if(!supported)return{kind:'unsupported',label:'Avisos no disponibles',copy:'Este navegador no admite notificaciones push.'};if(Notification.permission==='denied')return{kind:'blocked',label:'Avisos bloqueados',copy:'Actívalos desde los permisos del navegador.'};if(Notification.permission==='granted')return{kind:'ready',label:'Avisos activados',copy:'Los pedidos nuevos pueden llegar aunque cierres el panel.'};return{kind:'action',label:'Activa avisos de pedidos',copy:'Recibe una alerta cuando entre un pedido nuevo.'}}
