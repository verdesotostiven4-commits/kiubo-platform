...
async function syncItem(item,row=null){if(!item?.id||!token()||!navigator.onLine)return false;const current=attempts.get(item.id)||0;if(current>=MAX_ATTEMPTS){markPending(row||rowFor(item.id),'Foto pendiente · se volverá a intentar al reconectar o abrir el panel');return false}row=row||rowFor(item.id);if(row?.dataset.v10ImageSync==='busy')return false;if(row)row.dataset.v10ImageSync='busy';attempts.set(item.id,current+1);if(row)row.dataset.v10ImageAttempts=String(current+1);try{await post(item);forget(item.id);markSaved(row||rowFor(item.id));return true}catch(err){console.error('presentation image sync',err);markPending(row||rowFor(item.id));scheduleRetry(item.id);return false}}
...
function resetAttemptsAndFlush(){attempts.clear();retryTimers.forEach(t=>clearTimeout(t));retryTimers.clear();$$('#v5PresentationRows .v5-presentation-row[data-v77-pending-image="1"]').forEach(row=>{delete row.dataset.v10ImageSync;delete row.dataset.v10ImageAttempts});scan()}
