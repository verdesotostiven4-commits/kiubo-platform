...
const attempts=new Map();let flushing=false;
...
function forget(id){const q=readQueue();if(q[id]){delete q[id];writeQueue(q)}attempts.delete(id)}
...
function markSaved(row){if(!row)return;row.dataset.v77PendingImage='0';row.dataset.v10ImageSync='done';delete row.dataset.v10ImageAttempts;const copy=$('.v77-presentation-image-copy small',row);if(copy)copy.textContent='Foto guardada para esta presentación'}
function markPending(row,text='Foto pendiente de sincronizar · se reintentará automáticamente'){if(!row)return;row.dataset.v10ImageSync='error';const copy=$('.v77-presentation-image-copy small',row);if(copy)copy.textContent=text}
async function post(item){const session=token();if(!session)return{ok:false,reason:'no_session'};const r=await fetch(C.apiUrl,{method:'POST',headers:{'Content-Type':'application/json','X-Client-Version':C.version||'10.3.1','X-Provider-Session':session},body:JSON.stringify({action:'save_presentation_image',slug,presentation_id:item.id,image_url:item.image_url,image_path:item.image_path})});const b=await r.json().catch(()=>({}));if(!r.ok||!b?.ok)throw new Error(b?.error||'sync_failed');return{ok:true}}
async function syncItem(item,row=null){if(!item?.id)return false;const current=attempts.get(item.id)||0;if(current>=3)return false;if(!token())return false;row=row||rowFor(item.id);if(row?.dataset.v10ImageSync==='busy')return false;if(row)row.dataset.v10ImageSync='busy';attempts.set(item.id,current+1);try{await post(item);forget(item.id);markSaved(row||rowFor(item.id));return true}catch(err){console.error('presentation image sync',err);markPending(row||rowFor(item.id));return false}}
async function flushQueue(){if(flushing||!token())return;flushing=true;try{const q=readQueue();for(const item of Object.values(q)){if(!item?.id)continue;await syncItem(item)}}finally{flushing=false}}
...
function resetAttemptsAndFlush(){attempts.clear();$$('#v5PresentationRows .v5-presentation-row[data-v77-pending-image="1"]').forEach(row=>{delete row.dataset.v10ImageSync});scan()}
function start(){const modal=$('#productModal')||document.body;obs.observe(modal,{subtree:true,childList:true,attributes:true,attributeFilter:['data-v77-pending-image','src']});document.addEventListener('change',e=>{const row=e.target?.closest?.('.v5-presentation-row');if(row&&e.target?.closest?.('.v77-presentation-image')){attempts.delete(String(row.dataset.id||''));delete row.dataset.v10ImageSync;setTimeout(scan,120)}},true);document.addEventListener('click',e=>{if(e.target?.closest?.('#productModal'))setTimeout(scan,60)},true);window.addEventListener('online',resetAttemptsAndFlush);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')resetAttemptsAndFlush()});setTimeout(scan,80);setTimeout(scan,500);setTimeout(scan,1600)}
