(()=>{
  ...
  function polishPanel(){
    document.documentElement.classList.add('kiubo-panel-v7');decorateProductRows();injectDangerZone();
    const search=$('#productSearch');if(search)search.placeholder='Buscar producto, marca o código';
    const title=$('#viewTitle');if(title&&$('[data-nav="products"]')?.classList.contains('active'))title.textContent='Productos';
  }
  ...
  const start=()=>{polishPanel();new MutationObserver(polishPanel).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','class']})};
