...
export function FoodOrdersClientPro(){
  ...
  const registerPayment=(order:FoodOrderRecord)=>{
    ...
    let cashMovement:CashMovementRecord|undefined;if(paymentMethod==="cash"&&cashSession){const movementId=makeId("movement");cashMovement={id:movementId,tenantId:workspace.tenantId,branchId:workspace.branchId,sessionId:cashSession.id,type:"in",amount:applied,reason:creditPaymentCashMovementReason(payment.id,credit.description),clientOperationId:movementId,createdAt:now};next.cashMovements.unshift(cashMovement)}
