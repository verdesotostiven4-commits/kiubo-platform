...
export function OperationalSalesInsights(){
  ...
  const resetSales=async()=>{
    ...
    if(!resetConfigured){setResetMessage("Primero configura tu propio código de autorización.");return}
    const code=resetCode.trim();if(!code){setResetMessage("Escribe tu código de autorización.");return}
    ...
    const currentSettings=getTenantSettings(current,ctx.tenantId),visibleCount=current.sales.filter(s=>s.tenantId===ctx.tenantId&&s.branchId===ctx.branchId&&saleVisibleAfterHistoryReset(s,currentSettings)).length;
    if(!visibleCount){setResetMessage("El historial ya está vacío.");return}
    ...
    try{
      ...
      setResetCode("");setSelectedDay("");setResetOpen(true);setResetMessage(after.ok?`Ventas reiniciadas · ${visibleCount} registros de prueba quedaron archivados.`:"Ventas reiniciadas; KIUBO terminará de sincronizar el corte automáticamente.");
