...
export function SessionEnforcer(){
  ...
  useEffect(()=>{
    ...
    void(async()=>{
      if(path==="/"||path.startsWith("/login")||path.startsWith("/precios")||path.startsWith("/demo"))return;
      ...
      if((path.startsWith("/control")||path.startsWith("/leads"))&&!user.platformAdmin){
      ...
      if(session.activeTenantId!==tenantId||session.activeBranchId!==branch?.id){
        saveLocalSession({...session,tenantId:user.tenantId,activeTenantId:tenantId,activeBranchId:branch?.id});
      }

      if(!canAccess(user.role,permission))router.replace(homeForRole(user.role));
    })();
