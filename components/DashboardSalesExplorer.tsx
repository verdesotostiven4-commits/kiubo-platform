...
export function DashboardSalesExplorer({chart,total,trend,periodLabel}:{chart:ChartPoint[];total:number;trend:number;periodLabel:string}){
  ...
  return <article className="ref-white-card ref-week-chart dashboard-chart-card dashboard-area-explorer">
    ...
    {selected&&<div className="dashboard-drilldown-backdrop" role="presentation"><section className="dashboard-drilldown" role="dialog" aria-modal="true" aria-label={`Detalle de ${selected.label}`}><button className="dashboard-drilldown-close" type="button" onClick={()=>setSelectedKey("")} aria-label="Cerrar">×</button><div className="dashboard-drilldown-title"><span>DETALLE</span><h3>{selected.label}</h3><div><strong>{money(selected.total)}</strong><small>{selected.sales.length} {selected.sales.length===1?"venta":"ventas"}</small></div></div>{selected.sales.length?<div className="dashboard-drilldown-list">{[...selected.sales].sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).slice(0,12).map(sale=><article key={sale.id}><div><strong>{new Date(sale.createdAt).toLocaleTimeString("es-EC",{hour:"2-digit",minute:"2-digit"})}</strong><span>{paymentLabel[sale.payment]}</span></div><b>{money(sale.total)}</b><small>{sale.items.slice(0,4).map(item=>`${item.qty}× ${parseOperationalItemName(item.name).displayName}`).join(" · ")}{sale.items.length>4?` · +${sale.items.length-4}`:""}</small></article>)}</div>:<div className="dashboard-drilldown-empty">No hubo ventas en este período.</div>}</section></div>}
  </article>;
}
