...
const yogurtLike=(label:string)=>slug(label).startsWith("yogur");
const fallbackAsset=(choice:string,label:string)=>yogurtLike(label)?YOGURT_ASSETS[slug(choice)]||DEFAULT_YOGURT:undefined;

type Props={productName:string;config:ProductOptionConfig;choices:ProductOptionChoice[];onClose:()=>void;onConfirm:(selections:string[])=>void};

export function ProductOptionPicker({productName,config,choices,onClose,onConfirm}:Props){
  ...
  const imageMap=useMemo(()=>new Map(choices.map(choice=>[choice.label,choice.imageUrl||fallbackAsset(choice.label,config.label)])),[choices,config.label]);
  useEffect(()=>{for(const src of new Set([...imageMap.values()].filter((value):value is string=>Boolean(value)))){const image=new Image();image.src=src}},[imageMap]);
  ...
  return <div className="option-picker-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
    <section className="option-picker" role="dialog" aria-modal="true" aria-label={`Personalizar ${productName}`}>
      ...
      <div className={`option-picker-slots option-picker-slots-${Math.min(count,4)}`}>
        {selections.map((selection,index)=>{const src=selection?imageMap.get(selection):yogurtLike(config.label)?DEFAULT_YOGURT:undefined;return <button key={index} className={`option-picker-slot ${active===index?"active":""} ${selection?"filled":""}`} onClick={()=>setActive(index)}>
