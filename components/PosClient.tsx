...
export function PosClient(){
  ...
  const ctx=getWorkspaceContext(db),tenant=ctx.tenant,branch=ctx.branch;
  ...
  const filtered=products.filter(p=>`${p.name} ${p.barcode} ${p.price}`.toLowerCase().includes(query.toLowerCase()));
  ...
  const checkout=()=>{
    ...
    const sale:SaleRecord={
      ...
      payment,
      items:cart.map(line=>({productId:line.id,name:line.name,qty:line.qty,unitPrice:line.price,unitCost:line.cost})),
      createdAt:now,
    };
    ...
    const productSnapshots:TenantProduct[]=[];
    for(const line of cart){
    ...
    enqueueSaleTransaction(next,{sale,productSnapshots,stockMovements,credit});
    saveLocalDatabase(next,{trackChanges:false});
    setDb(next);
    setCart([]);
    setCustomerId("");
    setMessage(`Venta guardada · ${paymentLabel[payment]} · $${sale.total.toFixed(2)}`);
  };
  ...
  return <>
    <div className="workspace-banner">
      ...
      <div><span>ESTADO</span><strong>{tenant?.status.toUpperCase()}</strong></div>
    </div>
    <div className="pos-grid">
      <section className="pos-products">
        <div className="pos-message">{message}</div>
        <input className="search-input" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar por nombre, código o precio…"/>
        <div className="product-results">
          {filtered.length?filtered.map(product=><button className="pos-product" key={product.id} onClick={()=>add(product)}><div className="product-icon">{product.name.charAt(0)}</div><div><strong>{product.name}</strong><span>{product.barcode} · Stock {product.stock}</span></div><b>${product.price.toFixed(2)}</b></button>):<p className="empty-cart">Esta sucursal todavía no tiene productos. Agrégalos desde Catalog.</p>}
        </div>
      </section>
      <aside className="cart-panel">
        <div className="cart-head"><span className="eyebrow">VENTA ACTUAL</span><h2>{cart.reduce((n,l)=>n+l.qty,0)} productos</h2></div>
