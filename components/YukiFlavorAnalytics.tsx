...
export function YukiFlavorAnalytics(){
  ...
  if(!data)return null;
  const max=Math.max(1,data.ranking[0]?.qty||1);
  return <section className={styles.card} aria-label="Sabores de yogurt más vendidos">
    ...
    {data.ranking.length?<div className={styles.body}><div className={styles.hero}><span>Favorito del período</span><strong>{data.ranking[0].name}</strong><small>{data.ranking[0].qty} selecciones</small></div><div className={styles.list}>{data.ranking.slice(0,6).map((item,index)=><div className={styles.row} key={item.name}><b>{index+1}</b><div><div className={styles.rowHead}><strong>{item.name}</strong><span>{item.qty}</span></div><div className={styles.track}><i style={{width:`${Math.max(8,item.qty/max*100)}%`}}/></div></div></div>)}</div><div className={styles.summary}><span><b>{data.selections}</b> sabores seleccionados</span><span><b>{data.salesWithFlavor}</b> ventas con yogurt</span></div></div>:<div className={styles.empty}><strong>Aún no hay datos de sabores en este período.</strong><span>Cuando se registren ventas con yogurt, KIUBO mostrará aquí el ranking real.</span></div>}
  </section>;
}
