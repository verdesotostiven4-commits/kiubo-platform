...
export function OrderPrintClient(){
  ...
  useEffect(()=>{
    ...
    setData({order,business:branding.businessName||ctx.tenant?.name||"Negocio",logo:branding.logoUrl||"",branch:ctx.branch?`${ctx.branch.code} · ${ctx.branch.name}`:"Matriz",operator});
  },[]);
  ...
  return <main className="command-page"><style>{`
    ...
    *{box-sizing:border-box}
    ...
    .notes{margin-top:1.5mm;padding:2.2mm;border:2px solid #111;font-size:11px;font-weight:900;line-height:1.3}.notes-title{display:block;font-size:8px;letter-spacing:.12em;margin-bottom:1mm}
