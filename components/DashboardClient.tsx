...
export function DashboardClient(){
  ...
  return <div className="ref-business-dashboard dashboard-v3">
    ...
    <section className="ref-dashboard-title dashboard-title-v3"><div><span className="dashboard-live-dot">● EN VIVO</span><h1>Inicio</h1><p>{tenant.name} · {ctx.branch?.name||"Matriz"}</p></div><div className="dashboard-period-summary"><span>{rangeCopy[range].title}</span><strong>{money(dashboard.total)}</strong><small className={dashboard.trend>=0?"up":"down"}>{dashboard.trend>=0?"↗":"↘"} {Math.abs(dashboard.trend).toFixed(0)}% vs. período anterior</small></div></section>

    <section className="dashboard-period-toolbar" aria-label="Cambiar período del tablero">
      <div className="dashboard-period-copy"><span>VER RESULTADOS DE</span><strong>{rangeCopy[range].title}</strong></div>
      <div className="dashboard-range-switch dashboard-range-switch-v3">{(Object.keys(rangeCopy) as DashboardRange[]).map(key=><button key={key} aria-pressed={range===key} className={range===key?"active":""} onClick={()=>setRange(key)}><b>{rangeCopy[key].button}</b><small>{rangeCopy[key].hint}</small></button>)}</div>
    </section>

    <section className="dashboard-period-insights dashboard-range-enter" key={`insights-${range}`}>
      <article><span>Ventas</span><strong>{money(dashboard.total)}</strong><small>{rangeCopy[range].short}</small></article>
    ...
    <section className="dashboard-insight-grid dashboard-range-enter" key={`charts-${range}`}><article className="ref-white-card ref-week-chart dashboard-chart-card"><div className="ref-card-title"><div><span>Movimiento de ventas</span><small>{rangeCopy[range].title}</small></div><Link href="/reports">Abrir reporte ↗</Link></div><div className="dashboard-chart-total"><strong>{money(dashboard.total)}</strong><span className={dashboard.trend>=0?"up":"down"}>{dashboard.trend>=0?"▲":"▼"} {Math.abs(dashboard.trend).toFixed(1)}%</span></div><div className="ref-svg-chart"><svg viewBox="0 0 700 210" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id={`dashFill-${range}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#1d8658" stopOpacity=".28"/><stop offset="1" stopColor="#1d8658" stopOpacity="0"/></linearGradient></defs><polyline points={dashboard.chart.map((d,i)=>`${i*(700/Math.max(1,dashboard.chart.length-1))},${190-(d.total/max)*150}`).join(" ")} fill="none" stroke="#176a4a" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"/><polygon points={`0,210 ${dashboard.chart.map((d,i)=>`${i*(700/Math.max(1,dashboard.chart.length-1))},${190-(d.total/max)*150}`).join(" ")} 700,210`} fill={`url(#dashFill-${range})`}/></svg><div className="ref-chart-labels dashboard-chart-labels">{dashboard.chart.map(d=><span key={d.key}>{d.label}</span>)}</div></div></article>

      <article className="ref-white-card dashboard-donut-card dashboard-donut-v3"><div className="ref-card-title"><div><span>Cómo te pagaron</span><small>{rangeCopy[range].title}</small></div><button className="dashboard-info" type="button" aria-label="Cómo funciona este gráfico" aria-expanded={paymentHelpOpen} onClick={()=>setPaymentHelpOpen(v=>!v)}>i</button></div>{paymentHelpOpen&&<div className="dashboard-help-popover"><strong>Qué muestra este gráfico</strong><span>Reparte las ventas del período por efectivo, transferencia, fiado y pago mixto. Cambia automáticamente cuando eliges otro período arriba.</span></div>}<div className="dashboard-payment-highlight"><span>Principal</span><strong>{dominant?.label||"Sin ventas"}</strong><small>{dominant?`${dominant.pct.toFixed(1)}% del total`:"Aún sin datos"}</small></div><div className="dashboard-donut-wrap"><div className="dashboard-donut" style={{background:donut}}><div><small>Total</small><strong>{money(dashboard.total)}</strong><em>{dashboard.inPeriod.length} ventas</em></div></div><div className="dashboard-payment-legend">{payments.map(item=><div key={item.key}><i className={item.key}/><span>{item.label}<small>{item.pct.toFixed(1)}%</small></span><strong>{money(item.amount)}</strong></div>)}</div></div></article>

      <article className="ref-white-card ref-best-products dashboard-top-products"><div className="ref-card-title"><div><span>Más vendidos</span><small>{rangeCopy[range].short}</small></div><Link href="/reports">Ver todos</Link></div>{dashboard.top.length?dashboard.top.map((item,index)=><div className="ref-product-rank" key={`${item.name}-${index}`}><b>{item.name}</b><div><i style={{width:`${Math.max(20,100-index*16)}%`}}/></div><strong>{item.qty}</strong></div>):<div className="ref-empty-state"><b>Sin ventas todavía</b><span>Los favoritos aparecerán aquí.</span></div>}</article></section>
