...
export function ReceiptClient(){
  ...
  `}</style><section className="receipt">
    ...
    <div className="ticket-title">{internalUnits===itemCount&&itemCount>0?"CONSUMO INTERNO":"TICKET DE VENTA"}</div>{order&&<div className="order-number">PEDIDO #{String(order.number).padStart(4,"0")}</div>}{order&&<div className="service">{modeLabel[order.serviceMode]}{order.tableLabel?(order.serviceMode==="delivery"?` · DOMICILIO ${order.tableLabel}`:` · MESA ${order.tableLabel}`):""}</div>}<div className="summary-chip">{itemCount} {itemCount===1?"unidad":"unidades"} · {paymentLabel[sale.payment]}</div>
    ...
    <div className="rule"/><section className="totals"><div className="row"><span>Subtotal</span><b>{money(subtotal+discountTotal)}</b></div>{courtesyUnits>0&&<div className="row"><span>Cortesías</span><b>{courtesyUnits} u. · $0.00</b></div>}{internalUnits>0&&<div className="row"><span>Consumo interno</span><b>{internalUnits} u. · $0.00</b></div>}{discountTotal>0&&<div className="row"><span>Descuento</span><b>- {money(discountTotal)}</b></div>}<div className="row payment"><span>Forma de pago</span><b>{paymentLabel[sale.payment]}</b></div>{data.paymentBreakdown&&<><div className="row payment-split"><span>↳ Efectivo</span><b>{money(data.paymentBreakdown.cash)}</b></div><div className="row payment-split"><span>↳ Transferencia</span><b>{money(data.paymentBreakdown.transfer)}</b></div></>}<div className="row grand"><span>TOTAL</span><span>{money(sale.total)}</span></div></section>
