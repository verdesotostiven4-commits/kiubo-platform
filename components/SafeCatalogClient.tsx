...
export function SafeCatalogClient() {
  ...
  const change = (field: keyof Draft) => (event: ChangeEvent<HTMLInputElement>) => {
    setDraft((current) => ({ ...current, [field]: event.target.value }));
  };

  const openNew = () => {
    setEditor({ kind: "new" });
    setDraft({ ...blankDraft(), category: categories[0] || "General" });
    setError("");
    setMessage("");
      ...
      <section className={styles.layout}>
        <div className={styles.listPanel}>
          <div className={styles.panelHead}>
            <div>
              <span>INVENTARIO DE SUCURSAL</span>
              <h2>Mis productos</h2>
            </div>
          ...
          <div className={styles.productList}>
            {visibleProducts.map((product) => (
              <button
                key={product.id}
                type="button"
                className={`${styles.productRow} ${editor?.kind === "edit" && editor.id === product.id ? styles.selected : ""}`}
                onClick={() => openEdit(product)}
              >
                <span className={styles.photo}>
                  {product.imageUrl ? <img src={product.imageUrl} alt="" loading="lazy" /> : product.name.charAt(0).toUpperCase()}
                </span>
                <span className={styles.productInfo}>
                  <strong>{product.name}</strong>
                  <small>{product.category || "General"} · {product.barcode}</small>
                </span>
        ...
        <div className={styles.editorPanel} id="product-editor">
          <div className={styles.editorHead}>
            <div>
              <span className={styles.kicker}>{editor?.kind === "edit" ? "EDITAR PRODUCTO" : "NUEVO PRODUCTO"}</span>
              <h2>{editor ? (editor.kind === "edit" ? "Edita y guarda" : "Crea un producto") : "Editor de producto"}</h2>
              <p>{editor ? "Todos los campos están aquí, incluida la foto." : "Pulsa “Nuevo producto” o selecciona uno de la lista."}</p>
            </div>
          ...
          <form className={styles.form} onSubmit={saveProduct}>
            <label>
              Nombre del producto
              <input value={draft.name} onChange={change("name")} disabled={!editor || saving} maxLength={160} />
            </label>
            ...
            <label>
              ...
              <input
                ...
                list="kiubo-product-categories"
                maxLength={80}
                placeholder="Ej. Yogurts"
              />
              <datalist id="kiubo-product-categories">
            ...
            <div className={styles.twoCols}>
              <label>
                Costo
                <input type="number" min="0" step="0.0001" value={draft.cost} onChange={change("cost")} disabled={!editor || saving} />
              </label>
              <label>
                Precio de venta
                <input type="number" min="0" step="0.01" value={draft.price} onChange={change("price")} disabled={!editor || saving} />
              </label>
            </div>

            <label>
              {tracksStock ? "Stock" : "Stock · producto elaborado"}
              <input
                type="number"
                min="0"
                step="0.001"
                value={tracksStock ? draft.stock : "0"}
                onChange={change("stock")}
                disabled={!editor || saving || !tracksStock}
              />
              {!tracksStock && <small className={styles.hint}>Se vende normalmente y no muestra un conteo unitario.</small>}
            </label>

            <label>
              Foto desde la laptop
              <input
                key={fileInputKey}
                name="imageFile"
                type="file"
                ...
                disabled={!editor || saving}
              />
              <small className={styles.hint}>Para YUKI: horizontal 4:3. KIUBO optimiza la imagen antes de subirla.</small>
            </label>
            ...
            {editor && draft.imageUrl && (
              <div className={styles.preview}>
                <img src={draft.imageUrl} alt="Vista previa del producto" />
              </div>
            )}

            <button className={styles.save} type="submit" disabled={!editor || saving}>
              {saving ? "Guardando…" : editor?.kind === "edit" ? "Guardar cambios" : "Guardar producto"}
            </button>
