...
export function Sidebar(){
  ...
  const visible=useMemo(()=>items.filter(item=>!user||canAccess(user.role,item.permission,Boolean(user.platformAdmin))).filter(item=>!item.platformOnly||user?.platformAdmin).filter(item=>!platformPath||item.group==="platform").filter(item=>user?.platformAdmin||!item.feature||hasFeature(tenant,item.feature)),[platformPath,tenant,user]);
