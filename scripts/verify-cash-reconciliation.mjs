...
for(const needle of ["reconcileCashSession","session.openingAmount+cashSales+creditCollections+manualIncome-cashOut","sessionId===session.id","s.payment===\"cash\"","isCreditPaymentCashMovement"]){assert.ok(helper.includes(needle),`cash reconciliation helper missing: ${needle}`)}
const cash=text("components/CashClient.tsx");
for(const needle of ["reconcileCashSession","Ventas efectivo","Abonos efectivo","Otros ingresos","Egresos","Esperado","CUADRADA","sobrante","faltante"]){assert.ok(cash.includes(needle),`CashClient missing reconciliation guard: ${needle}`)}
