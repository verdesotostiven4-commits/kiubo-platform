...
def run_psql(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(["psql", URL, "-v", "ON_ERROR_STOP=1", *args], cwd=ROOT, text=True, capture_output=True)
...
def main() -> None:
    ...
    try:
        p = urlparse(URL)
        line(lines, "scheme", p.scheme)
        line(lines, "host", p.hostname)
        line(lines, "port", p.port)
        line(lines, "user", p.username)
        line(lines, "database", (p.path or "").lstrip("/"))
        line(lines, "password_present", "yes" if p.password else "no")
    ...
    code, count, err = scalar(f"select count(*) from information_schema.tables where table_schema='public' and table_name in ({table_list});")
    if code != 0:
    ...
    if count == "0":
        ...
        combined.write_text("\n\n".join(path.read_text(encoding="utf-8") for path in migration_paths), encoding="utf-8")
    ...
    _, tables, table_err = scalar(f"select count(*) from information_schema.tables where table_schema='public' and table_name in ({table_list});")
    fn_list = ",".join(repr(x) for x in EXPECTED_FUNCTIONS)
    _, functions, fn_err = scalar(f"select count(distinct proname) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and proname in ({fn_list});")
    _, rls, rls_err = scalar(f"select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname in ({table_list}) and c.relrowsecurity;")

    if table_err or fn_err or rls_err:
        line(lines, "status", "FAILED_VERIFY")
        line(lines, "error", safe_error(table_err or fn_err or rls_err))
    else:
