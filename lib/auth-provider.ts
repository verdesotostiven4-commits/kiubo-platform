import { clearLocalSession, getLocalDeviceId, getPrimaryBranch, getWorkspaceContext, loadLocalDatabase, loadLocalSession, saveLocalSession, type LocalSession, type UserRecord } from "./local-store";
...
import { releaseOperationalSession } from "./operational-session";
...
const cloudAuthProvider:KiuboAuthProvider={
  ...
  async signOut(){const client=getSupabaseBrowserClient();if(client){try{const ctx=getWorkspaceContext(loadLocalDatabase());if(ctx.user&&!ctx.user.platformAdmin&&ctx.tenantId)await releaseOperationalSession(ctx.tenantId,getLocalDeviceId())}catch{}await client.auth.signOut()}clearLocalSession()},
