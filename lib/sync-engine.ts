...
import { getSyncSummary, loadLocalDatabase, saveLocalDatabase, updateSyncOperation } from "./local-store";
...
function keyFor(entity:SyncEntity,record:Record<string,unknown>){return entity==="settings"?String(record.tenantId||""):String(record.id||"")}
...
export async function runSyncCycle():Promise<SyncCycleResult>{
  ...
  const db=loadLocalDatabase(),syncableTenants=new Set(db.tenants.filter(t=>t.plan!=="Internal").map(t=>t.id)),pending=db.syncQueue.filter(item=>syncableTenants.has(item.tenantId)&&(item.status==="pending"||item.status==="failed")).slice(0,100);
  if(!pending.length){const pulled=await provider.pullChanges(db.syncCursor);if(pulled.changes.length){const mutable=db as unknown as Record<SyncEntity,Record<string,unknown>[]>;for(const change of pulled.changes){const collection=mutable[change.entityType];if(!Array.isArray(collection))continue;const index=collection.findIndex(record=>keyFor(change.entityType,record)===change.entityId);if(change.action==="delete"){if(index>=0)collection.splice(index,1);continue}if(!change.payload||typeof change.payload!=="object")continue;const record=change.payload as Record<string,unknown>;if(index>=0)collection[index]=record;else collection.push(record)}}db.syncCursor=pulled.cursor??db.syncCursor;saveLocalDatabase(db,{trackChanges:false});return{ok:true,mode:provider.mode,pushed:0,failed:0,pulled:pulled.changes.length,message:"Datos cloud actualizados"}}
  ...
  const pulled=await provider.pullChanges(afterPush.syncCursor);if(pulled.changes.length){const mutable=afterPush as unknown as Record<SyncEntity,Record<string,unknown>[]>;for(const change of pulled.changes){const collection=mutable[change.entityType];if(!Array.isArray(collection))continue;const index=collection.findIndex(record=>keyFor(change.entityType,record)===change.entityId);if(change.action==="delete"){if(index>=0)collection.splice(index,1);continue}if(!change.payload||typeof change.payload!=="object")continue;const record=change.payload as Record<string,unknown>;if(index>=0)collection[index]=record;else collection.push(record)}}afterPush.syncCursor=pulled.cursor??afterPush.syncCursor;saveLocalDatabase(afterPush,{trackChanges:false});return{ok:true,mode:provider.mode,pushed,failed,pulled:pulled.changes.length,message:"Sincronización completada"};
}

export function syncSnapshot(){return getSyncSummary(loadLocalDatabase())}
