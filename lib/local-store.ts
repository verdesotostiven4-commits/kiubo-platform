...
export type TenantProduct = { id:string; tenantId:string; branchId:string; masterProductId:string; barcode:string; name:string; price:number; cost:number; stock:number; active:boolean };
...
export type SaleRecord = { id:string; tenantId:string; branchId:string; customerId?:string; total:number; payment:"cash"|"transfer"|"mixed"|"credit"; items:{productId:string;name:string;qty:number;unitPrice:number;unitCost?:number}[]; clientOperationId?:string; createdAt:string };
...
export type TenantSettings = { tenantId:string; tradeName:string; legalName:string; ruc:string; establishment:string; emissionPoint:string; currency:"USD"; accent:string; receiptFooter:string; requireCashSession:boolean; allowCredit:boolean };
...
export type KiuboLocalDatabase = {
  ...
  customers:CustomerRecord[]; sales:SaleRecord[]; users:UserRecord[]; cashSessions:CashSessionRecord[];
...
const defaultSettings=(tenantId:string,tradeName:string):TenantSettings=>({tenantId,tradeName,legalName:"",ruc:"",establishment:"001",emissionPoint:"001",currency:"USD",accent:"#ff5b55",receiptFooter:"Gracias por tu compra",requireCashSession:false,allowCredit:true});
...
export const initialDatabase:KiuboLocalDatabase={
  ...
  tenantProducts:[
    {id:"tp-1",tenantId:PILOT_TENANT_ID,branchId:PILOT_BRANCH_ID,masterProductId:"m-1",barcode:"786000000001",name:"Leche Entera 1L",price:1.25,cost:.92,stock:18,active:true},
    {id:"tp-2",tenantId:PILOT_TENANT_ID,branchId:PILOT_BRANCH_ID,masterProductId:"m-2",barcode:"786000000002",name:"Café Buen Día",price:.15,cost:.10,stock:42,active:true},
    {id:"tp-3",tenantId:PILOT_TENANT_ID,branchId:PILOT_BRANCH_ID,masterProductId:"m-3",barcode:"786000000003",name:"Agua 600ml",price:.75,cost:.50,stock:30,active:true},
    {id:"tp-4",tenantId:PILOT_TENANT_ID,branchId:PILOT_BRANCH_ID,masterProductId:"m-4",barcode:"786000000004",name:"Galletas",price:.50,cost:.32,stock:25,active:true}
  ],
...
const TRACKED_COLLECTIONS:SyncEntity[]=["tenants","branches","tenantProducts","customers","sales","users","cashSessions","cashMovements","credits","creditPayments","settings","branding","suppliers","purchases","supplierPayments","stockMovements"];
...
function normalize(raw:Partial<KiuboLocalDatabase>|null|undefined):KiuboLocalDatabase{
  ...
  return{
    ...
    sales,users,cashSessions,cashMovements,credits,creditPayments,settings,branding,
...
export function createLocalBackup(){return JSON.stringify({format:"kiubo-local-backup-v5",exportedAt:new Date().toISOString(),deviceId:getLocalDeviceId(),database:loadLocalDatabase()},null,2)}
